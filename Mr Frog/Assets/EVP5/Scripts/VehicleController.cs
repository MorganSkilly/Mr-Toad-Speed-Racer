...
public class VehicleController : MonoBehaviour
	...
	public Vector3 frontAxlePosition { get { return transform.TransformPoint(new Vector3 (0.0f, m_vehicleFrame.baseHeight, m_vehicleFrame.frontPosition)); } }
	public Vector3 rearAxlePosition { get { return transform.TransformPoint(new Vector3 (0.0f, m_vehicleFrame.baseHeight, m_vehicleFrame.rearPosition)); } }
	...
	void UpdateTransform (WheelData wd, float deltaTime)
		...
		if (wd.wheel.wheelTransform != null || wd.wheel.caliperTransform != null)
			...
			if (wheelPositionMode == PositionMode.Fast)
				...
				{
				wd.visualGrounded = Physics.Raycast(wd.origin, -wd.transform.up, out wd.visualHit, (wd.collider.suspensionDistance + wd.collider.radius), Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);

				if (wd.visualGrounded)
					{
					// If these layers are intended to ignore collisions then just use the actual WheelHit information

					if (Physics.GetIgnoreLayerCollision(wd.collider.gameObject.layer, wd.visualHit.collider.gameObject.layer))
						{
						elongation = wd.collider.suspensionDistance * (1.0f - wd.suspensionCompression) + wd.collider.radius * 0.05f;
						wd.visualHit.point = wd.hit.point;
						wd.visualHit.normal = wd.hit.normal;
	...
	public void ResetVehicle ()
		...
		m_rigidbody.MovePosition(new Vector3(0, 0.4f, m_rigidbody.position.z) + Vector3.up * 1.6f);
	...
	void ProcessContacts (Collision col, bool isCollisionEnter)
		...
		foreach (ContactPoint contact in col.contacts)
			...
			if (dragRatio < -impactThreeshold || isCollisionEnter && col.relativeVelocity.sqrMagnitude > sqrImpactSpeed)
				{
				// Impact
