...
public class VehicleDamage : MonoBehaviour
	...
	void OnEnable ()
		...
		for (int i = 0; i < meshes.Length; i++)
			...
			if (mesh == null)
				{
				Debug.LogWarning(this.name + ": A mesh is missing! (" + meshes[i].name + ") Mesh damage disabled.", meshes[i].gameObject);
		...
		for (int i = 0; i < colliders.Length; i++)
			...
			if (mesh == null)
				{
				Debug.LogWarning(this.name + ": A mesh collider is missing! (" + colliders[i].name + ") Collider damage disabled.", colliders[i].gameObject);
				colliders = new MeshCollider[0];
				break;
				}
	...
	void Update ()
		{
		if (enableRepairKey && (Input.GetKeyDown(repairKey) || Input.GetKeyDown(KeyCode.Return)))
			m_repairing = true;
